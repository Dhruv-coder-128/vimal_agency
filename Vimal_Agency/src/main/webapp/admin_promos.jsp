                    ...
                    <%@ include file="admin_header.jsp" %>
                        ...
                        <div class="admin-main p-4">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <div>
                                    <h1 style="font-weight: 800; color: #1a242f; margin: 0; letter-spacing: -1px;">Promo
                                        Codes</h1>
                                    <p style="color: #7f8c8d; margin: 0;">Manage targeted category discounts for Vimal
                                        Agency.</p>
                                </div>
                            ...
                            <% String successMsg=null; // Emoji support mate UTF-8 connection string Connection
                                con=DatabaseManager.getConnection(); if(request.getMethod().equalsIgnoreCase("POST")) {
                                request.setCharacterEncoding("UTF-8"); // Encoding set karyu String
                                action=request.getParameter("action"); if("add".equals(action)) { // Fixed: Input
                                name 'category_name' match karyu String
                                sql="INSERT INTO promo (code_name, discount_percentage, description, min_order_amount, category_name) VALUES (?, ?, ?, ?, ?)"
                                ; PreparedStatement ps=con.prepareStatement(sql); ps.setString(1,
                                request.getParameter("code_name")); ps.setString(2, request.getParameter("discount"));
                                ps.setString(3, request.getParameter("description")); ps.setString(4,
                                request.getParameter("min_amount")); ps.setString(5,
                                request.getParameter("category_name")); ps.executeUpdate();
                                successMsg="Promo created for " + request.getParameter("category_name") + " category!" ;
                                } else if("update".equals(action)) { String
                                sql="UPDATE promo SET code_name=?, discount_percentage=?, description=?, min_order_amount=?, category_name=? WHERE id=?"
                                ; PreparedStatement ps=con.prepareStatement(sql); ps.setString(1,
                                request.getParameter("code_name")); ps.setString(2, request.getParameter("discount"));
                                ps.setString(3, request.getParameter("description")); ps.setString(4,
                                request.getParameter("min_amount")); ps.setString(5,
                                request.getParameter("category_name")); ps.setString(6,
                                request.getParameter("promo_id")); ps.executeUpdate();
                                successMsg="Promo updated successfully!" ; } } if(request.getParameter("del_id") !=null)
                                { PreparedStatement ps=con.prepareStatement("DELETE FROM promo WHERE id=?");
                                ps.setString(1, request.getParameter("del_id")); ps.executeUpdate();
                                    ...
                                    <table class="table table-hover align-middle mb-0 text-center">
                                        ...
                                        <tbody>
                                            <% ResultSet rs=con.createStatement().executeQuery("SELECT * FROM promo
                                                ORDER BY id DESC"); while(rs.next()) { %>
                                                <tr style="border-bottom: 1px solid #f1f1f1;">
                                                    <td class="fw-bold">#<%= rs.getInt("id") %>
                                                    </td>
                                                    <td><span class="promo-code-badge">
                                                    ...
                                                    <td class="fw-bold">₹<%= rs.getDouble("min_order_amount") %>
                                                    </td>
                                                    <td>
                                                        <div class="action-flex">
                                                            ...
                                                            <a href="admin_promos.jsp?del_id=<%= rs.getInt(" id") %>"
                                ...
                                <div class="modal-content" style="border-radius: 15px;">
                                    ...
                                    <form method="POST">
                                        ...
                                        <div class="modal-body p-4">
                                            <div class="row g-3">
                                                <div class="col-md-6"><label class="fw-bold mb-1">Code
                                                        Name</label><input type="text" name="code_name"
                                                        id="edit_code_name" class="form-control" required></div>
                                                <div class="col-md-6"><label class="fw-bold mb-1">Discount
                                                        (%)</label><input type="number" name="discount"
                                                        id="edit_discount" class="form-control" required></div>
                                                <div class="col-12">
                                                ...
                                                <div class="col-12"><label class="fw-bold mb-1">Min. Amount
                                                        (₹)</label><input type="number" name="min_amount"
                                                        id="edit_min_amount" class="form-control" required></div>
                                                <div class="col-12"><label
                                                        class="fw-bold mb-1">Description</label><textarea
                                                        name="description" id="edit_description" class="form-control"
                                                        rows="2"></textarea></div>
                                            </div>
                                        </div>
                                        <div class="modal-footer border-0"><button type="submit"
                                                class="btn btn-primary fw-bold px-4">Update Promo</button></div>
                                    </form>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <form method="POST">
                                        ...
                                        <div class="modal-body p-4">
                                            <div class="row g-3">
                                                <div class="col-md-6"><label class="fw-bold mb-1">Code
                                                        Name</label><input type="text" name="code_name"
                                                        class="form-control" placeholder="VIMAL20" required></div>
                                                <div class="col-md-6"><label class="fw-bold mb-1">Discount
                                                        (%)</label><input type="number" name="discount"
                                                        class="form-control" placeholder="20" required></div>
                                                <div class="col-12">
                                        ...
                                        <div class="modal-footer border-0"><button type="submit"
                                                class="btn btn-warning fw-bold px-4">Save Promo</button></div>
                                    </form>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="toast-body">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <strong class="text-success"
                                                style="font-size: 17px; white-space: nowrap;">Success:</strong>
