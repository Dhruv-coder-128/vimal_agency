...
<head>
    ...
    <style>
        ...
        .table td, .table th {
            padding: 10px 15px !important; /* Row height nani kari */
            font-size: 14px;
        ...
        .status-badge { font-weight: 700; display: flex; align-items: center; justify-content: center; }
        .dot { height: 7px; width: 7px; background-color: #28a745; border-radius: 50%; display: inline-block; margin-right: 6px; }
        
        /* Professional Single Line Toast */
        .toast-container-header {
            position: fixed; top: 25px; left: 50%; transform: translateX(-50%);
            z-index: 2500; width: max-content; min-width: 420px;
        }
        .toast-success-header { 
            border-left: 8px solid #28a745 !important; border-radius: 12px !important; 
            background: white !important; box-shadow: 0 10px 35px rgba(0,0,0,0.15) !important;
            padding: 10px 20px;
        }
    </style>
</head>
<body>
    ...
    <div class="admin-main p-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 style="font-weight: 800; color: #1a242f; margin: 0; letter-spacing: -1px;">Marketing Offers</h1>
                <p style="color: #7f8c8d; margin: 0;">Update your website banners and promotional text.</p>
            </div>
            <button type="button" class="btn btn-warning fw-bold px-4 py-2" data-bs-toggle="modal" data-bs-target="#addOfferModal" style="border-radius: 10px;">
                + Create New Offer
            </button>
        </div>
        ...
        <%
            ...
            if(request.getMethod().equalsIgnoreCase("POST") && "update".equals(request.getParameter("action"))) {
                ...
                ps.setString(3, request.getParameter("offer_id"));
                ps.executeUpdate();
            ...
            <table class="table table-hover align-middle mb-0 text-center">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f1f1f1;">
                        <td class="fw-bold px-4">#<%= rs.getInt("id") %></td>
                        <td class="offer-text-cell"><%= rs.getString("offer_text") %></td>
                        <td>
                        ...
                        <td>
                            <div class="action-flex">
                                <button type="button" class="btn-action btn-edit" data-bs-toggle="modal" data-bs-target="#editOfferModal" 
                                        onclick="fillEditModal('<%= rs.getInt("id") %>', '<%= rs.getString("offer_text") %>', '<%= isActive %>')">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <a href="admin_offers.jsp?del_id=<%= rs.getInt("id") %>" class="btn-action btn-delete" onclick="return confirm('Delete this offer?')">
