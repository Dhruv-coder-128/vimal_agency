...
<head>
    ...
    <style>
        :root { --primary: #4a2c7c; --secondary: #f0c14b; --bg: #f4f7f6; --success: #2ecc71; }
        body { font-family: 'Segoe UI', Tahoma, sans-serif; background: var(--bg); margin: 0; }
        .container { max-width: 900px; margin: 40px auto; padding: 20px; }
        .checkout-card { background: #fff; padding: 30px; border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.1); }
        h2 { color: var(--primary); border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 25px; }
        ...
        .col { flex: 1; }
        .order-summary { background: #f9f9f9; padding: 20px; border-radius: 8px; margin-bottom: 25px; border-left: 5px solid var(--primary); }
        .summary-line { display: flex; justify-content: space-between; margin: 8px 0; font-size: 15px; }
        .total-line { font-weight: bold; font-size: 1.2em; color: var(--success); border-top: 1px solid #ddd; padding-top: 10px; margin-top: 10px; }
        .btn-place-order { width: 100%; background: var(--success); color: white; border: none; padding: 15px; border-radius: 8px; font-size: 18px; font-weight: bold; cursor: pointer; transition: 0.3s; }
        .btn-place-order:hover { background: #27ae60; transform: translateY(-2px); }
    </style>
</head>
<body>
...
<%
    ...
    if(request.getMethod().equalsIgnoreCase("POST")) {
        try {
            ...
            // 1. Order insert karo ane Order ID melvo (RETURN_GENERATED_KEYS jaruri che)
            ...
            // Navi Generated Order ID fetch karo
            ...
            // 2. JO Order ID mali hoy, TO Cart mathi items 'order_items' ma copy karo
            if (newOrderId > 0) {
