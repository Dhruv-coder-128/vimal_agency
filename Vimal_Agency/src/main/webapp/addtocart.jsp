...
<%
    ...
    // ======================================================
    // Retrieve user_id from session object
    Object userIdObj = session.getAttribute("user_id");

    // If user_id is not found in session, return 401 Unauthorized status
    if(userIdObj == null) {
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        return; // Stop further execution
    }

    // Convert session object to integer userId
    ...
    // Product price (received as String)
    String priceStr = request.getParameter("price");

    // Product image path
    String img = request.getParameter("img");

    // Action type: can be 'add', 'minus', or 'delete'
    String action = request.getParameter("action");

    // Continue only if product name is provided
    if(name != null) {
        ...
        try {
            ...
            if ("add".equals(action) || action == null) {
                ...
                if(rs.next()) {
                    ...
                    PreparedStatement up = con.prepareStatement(
                        "UPDATE cart SET qty = qty + 1 WHERE user_id=? AND product_name=?"
                    );
                    ...
                    PreparedStatement ins = con.prepareStatement(
                        "INSERT INTO cart (user_id, product_name, price, image, qty) VALUES (?, ?, ?, ?, 1)"
                    );

                    ins.setInt(1, userId);
                    ins.setString(2, name);

                    // Convert price string into integer before storing
            ...
            // ======================================================
                ...
                // Decrease quantity only if qty > 1
