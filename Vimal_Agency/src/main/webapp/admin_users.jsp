            ...
            try {
                ...
                if (request.getMethod().equalsIgnoreCase("POST") && "update_user".equals(request.getParameter("action"))) {
                    ...
                    try (PreparedStatement ps = con.prepareStatement(sql)) {
                        ...
                        ps.setString(3, request.getParameter("password"));
                        ps.setString(4, request.getParameter("role"));
                        ps.setInt(5, Integer.parseInt(request.getParameter("status")));
                                ...
                                while (rs.next()) {
                                    ...
                                    if (joinedOnRaw != null) {
                                        try {
                                            ...
                                            finalDate = joinedOnRaw;
                                        }
                                    }

                                    String safeUser = (uName != null) ? uName.replace("'", "\\'").replace("\"", "&quot;") : "";
                                    String safeEmail = (uEmail != null) ? uEmail.replace("'", "\\'").replace("\"", "&quot;") : "";
                                    String safePass = (plainPass != null) ? plainPass.replace("'", "\\'").replace("\"", "&quot;") : "";
                    %>
                    <tr style="border-bottom: 1px solid #f1f1f1;">
                        <td class="fw-bold">#<%= uId %></td>
                        <td class="text-start"><%= (uName != null ? uName : "") %></td>
                        <td class="text-start text-muted"><%= (uEmail != null ? uEmail : "") %></td>
                        <td>
                            <div class="d-flex align-items-center justify-content-center">
                                <span id="pass_<%= uId %>" class="pass-hide me-2">••••••••</span>
                                <button type="button" class="btn btn-sm text-primary p-0" onclick="togglePass('<%= uId %>', '<%= safePass %>')" style="background:none; border:none;">
                                    <i id="icon_<%= uId %>" class="fa-solid fa-eye"></i>
                                </button>
                            </div>
                        </td>
                        <td><span class="<%= "admin".equalsIgnoreCase(role) ? "role-admin" : "role-user" %>"><%= (role != null ? role.toUpperCase() : "USER") %></span></td>
                        ...
                        <td>
                            <div class="action-flex">
                                <button type="button" class="btn-action btn-edit" data-bs-toggle="modal" data-bs-target="#editUserModal"
                                    onclick="fillUserModal('<%= uId %>', '<%= safeUser %>', '<%= safeEmail %>', '<%= safePass %>', '<%= (role != null ? role : "user") %>', '<%= status %>')">
                                    <i class="fa-solid fa-user-gear"></i>
                                </button>
                                <a href="admin_users.jsp?del_id=<%= uId %>" class="btn-action btn-delete" onclick="return confirm('Delete this user account?')">
            ...
            <div class="modal-content" style="border-radius: 15px;">
                ...
                <form method="POST">
                    ...
                    <div class="modal-footer border-0">
                        <button type="submit" class="btn btn-warning fw-bold px-4">Create Account</button>
                    </div>
                </form>
            </div>
        </div>
