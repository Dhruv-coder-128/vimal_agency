                                    ...
                                    <%@ include file="admin_header.jsp" %>
                                        ...
                                        <div class="admin-main p-4">
                                            <div class="d-flex justify-content-between align-items-center mb-4">
                                                <div>
                                                    <h1
                                                        style="font-weight: 800; color: #1a242f; margin: 0; letter-spacing: -1px;">
                                                        User Management</h1>
                                                    <p style="color: #7f8c8d; margin: 0;">Manage administrative roles
                                                        and customer accounts.</p>
                                                </div>
                                                <button type="button" class="btn btn-warning fw-bold px-4 py-2"
                                            ...
                                            <% String successMsg=null; Connection con=DatabaseManager.getConnection();
                                                // 1. INSERT NEW USER LOGIC
                                                if(request.getMethod().equalsIgnoreCase("POST") && "add_user"
                                                .equals(request.getParameter("action"))) { try { String
                                                sql="INSERT INTO users (username, email, password, role, status, created_at) VALUES (?, ?, ?, ?, 1, CURRENT_TIMESTAMP)"
                                                ; PreparedStatement ps=con.prepareStatement(sql); ps.setString(1,
                                                request.getParameter("username")); ps.setString(2,
                                                request.getParameter("email")); ps.setString(3,
                                                request.getParameter("password")); ps.setString(4,
                                                request.getParameter("role")); ps.executeUpdate(); successMsg="New " +
                                                request.getParameter("role").toUpperCase() + " added successfully!" ; }
                                                catch(Exception e) { out.println(e); } } // 2. UPDATE USER LOGIC
                                                (Sudharelo) if(request.getMethod().equalsIgnoreCase("POST")
                                                && "update_user" .equals(request.getParameter("action"))) { try { String
                                                sql="UPDATE users SET username=?, email=?, password=?, role=?, status=? WHERE id=?"
                                                ; PreparedStatement ps=con.prepareStatement(sql); ps.setString(1,
                                                request.getParameter("username")); ps.setString(2,
                                                request.getParameter("email")); ps.setString(3,
                                                request.getParameter("password")); ps.setString(4,
                                                request.getParameter("role")); ps.setString(5,
                                                request.getParameter("status")); ps.setString(6,
                                                request.getParameter("user_id")); ps.executeUpdate();
                                                successMsg="User account updated successfully!" ; } catch(Exception e) {
                                                out.println(e); } } // 3. DELETE USER LOGIC
                                                if(request.getParameter("del_id") !=null) { PreparedStatement
                                                ps=con.prepareStatement("DELETE FROM users WHERE id=?"); ps.setString(1,
                                                    ...
                                                    <table class="table table-hover align-middle mb-0 text-center">
                                                        ...
                                                        <tbody>
                                                            <% try { ResultSet
                                                                ...
                                                                users ORDER BY id DESC"); SimpleDateFormat dbFormat=new
                                                                SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                                                SimpleDateFormat userFormat=new
                                                                SimpleDateFormat("dd-MM-yyyy | hh:mm a");
                                                                while(rs.next()) { String role=rs.getString("role"); int
                                                                status=rs.getInt("status"); String
                                                                plainPass=rs.getString("password"); int
                                                                uId=rs.getInt("id"); String
                                                                joinedOnRaw=rs.getString("created_at"); String
                                                                finalDate="" ; if(joinedOnRaw !=null) { Date
                                                                date=dbFormat.parse(joinedOnRaw);
                                                                finalDate=userFormat.format(date); } %>
                                                                <tr style="border-bottom: 1px solid #f1f1f1;">
                                                                    <td class="fw-bold">#<%= uId %>
                                                                    </td>
                                                                    <td class="text-start">
                                                                    ...
                                                                    <td>
                                                                        <div
                                                                            ...
                                                                            <span id="pass_<%= uId %>"
                                                                                class="pass-hide me-2">••••••••</span>
                                                                            <button type="button"
                                                                                class="btn btn-sm text-primary p-0"
                                                                                onclick="togglePass('<%= uId %>', '<%= plainPass %>')"
                                                                    ...
                                                                    <td><span class="<%= " admin".equals(role)
                                                                            ? "role-admin" : "role-user" %>"><%=
                                                                                role.toUpperCase() %></span></td>
                                                                    <td><span class="<%= (status == 1) ? "
                                                                    ...
                                                                    <td
                                                                        ...
                                                                        <i
                                                                            class="fa-regular fa-clock me-1 text-warning"></i>
                                                                        <%= finalDate %>
                                                                    </td>
                                                                    ...
                                                                    <td>
                                                                        <div class="action-flex">
                                                                            <button type="button"
                                                                                ...
                                                                                onclick="fillUserModal('<%= uId %>', '<%= rs.getString("
                                                                                username") %>', '<%=
                                                                                    rs.getString("email") %>', '<%=
                                                                                        plainPass %>', '<%= role %>', '
                                                                                            <%= status %>')">
                                                                            ...
                                                                            <a href="admin_users.jsp?del_id=<%= uId %>"
                                                                                class="btn-action btn-delete"
                                                                                onclick="return confirm('Delete this user account?')"><i
                                                    ...
                                                    <form method="POST">
                                                        ...
                                                        <div class="modal-body p-4">
                                                            <div class="mb-3"><label
                                                                    class="fw-bold mb-1">Username</label><input
                                                                    type="text" name="username" class="form-control"
                                                                    placeholder="Enter username..." required></div>
                                                            <div class="mb-3"><label
                                                                    class="fw-bold mb-1">Email</label><input
                                                        ...
                                                        <div class="modal-footer border-0"><button type="submit"
                                                                class="btn btn-warning fw-bold px-4">Create
                                                                Account</button></div>
                                                    </form>
                                                </div>
                                            </div>
                                                ...
                                                <div class="modal-content" style="border-radius: 15px;">
                                                    ...
                                                    <form method="POST">
                                                        ...
                                                        <div class="modal-footer border-0">
                                                            <button type="submit"
                                                                class="btn btn-primary fw-bold px-4">Save
                                                                Changes</button>
                                                        </div>
                                                    </form>
                                                </div>
                                        ...
                                        <div class="toast-container-header">
                                            <div id="successToast"
                                                ...
                                                <div class="d-flex align-items-center p-2">
                                                    ...
                                                    <button type="button" class="btn-close ms-4 me-2"
